  ...
  switch (filterType) {
    case '&':
      parsedFilters.forEach((query, index) => {
        if (index === 0) {
          resultSet = databaseProvider.getAnd(query[0].trim(), query[1].trim(), initialDataSet, request);
        } else {
          resultSet = databaseProvider.getAnd(query[0].trim(), query[1].trim(), resultSet, request);
        }
      });
      break;
    default:
